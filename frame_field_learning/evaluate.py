...
def evaluate(gpu: int, config: dict, shared_dict, barrier, eval_ds, backbone):
    ...
    eval_sampler = torch.utils.data.Sampler(eval_ds)
    ...
    eval_ds = torch.utils.data.DataLoader(eval_ds, batch_size=config["optim_params"]["eval_batch_size"], pin_memory=True, num_workers=config["num_workers"])
    ...
    ##################### Prøve å teste på hele bilder
    #config["eval_params"]['patch_size'] = None
    #####################
