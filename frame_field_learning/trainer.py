...
from lydorn_utils import python_utils, print_utils
...
class Trainer:
    ...
    def fit(self, train_dl, val_dl=None, init_dl=None):
        ...
        if checkpoint is not None and checkpoint["epoch"] > self.config["optim_params"]["max_epoch"]:
            print_utils.print_failure("Max epoch set to {}, checkpoint epoch is {}. Change max epoch.".format(self.config["optim_params"]["max_epoch"], checkpoint["epoch"]))
...
"""
    ...
    def compute_loss_norms(self, dl, total_batches):
        ...
        batch_i = 0
        while batch_i < total_batches:
            print("batch {}/{}".format(batch_i, total_batches))
            for batch in dl:
                # Update loss norms
                batch = local_utils.batch_to_cuda(batch)
                pred, batch = self.model(batch)
    ...
    def fit(self, train_dl, val_dl=None, init_dl=None):
        ...
        for epoch in fit_iterator:
            ...
            self.model.train()
            train_loss, train_iou, iter_step = self.run_epoch("train", train_dl, epoch, self.config["optim_params"]["log_steps"], opt=self.optimizer)
