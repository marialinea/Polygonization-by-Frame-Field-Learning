...
# from torch_lydorn.torch.utils.data import Dataset as LydornDataset
from torch_lydorn.torchvision.datasets import utils

METADATA_DICT = {
        "fold": "train",
        "pixelsize": 0.08,
        "mean": [0.42441573, 0.39692576, 0.34374081],
        "std": [0.22762118, 0.21911445, 0.19840957],
}

IMAGE_DIRNAME = "images"
IMAGE_NAME_FORMAT = "{number}"
IMAGE_FILENAME_FORMAT = IMAGE_NAME_FORMAT + ".tif"  # City name, number


class Autokart(torch.utils.data.Dataset):
    """
    Autokart dataset
    """

    def __init__(self, root: str, fold: str="train", pre_process: bool=True, tile_filter=None, patch_size: int=None, patch_stride: int=None, pre_transform=None, transform=None, small: bool=False, pool_size: int=1, raw_dirname: str="raw", processed_dirname: str="processed", gt_source: str="disk", gt_type: str="tif", gt_dirname: str="ground_truth", mask_only: bool=False):
        """
        ...
        if self.pre_process:
            ...
            self.stats_filepath = os.path.join(self.processed_dirpath, "stats.pt")
            self.processed_flag_filepath = os.path.join(self.processed_dirpath, "processed_flag")
    ...
    def get_tile_info_list(self, tile_filter=None):
        tile_info_list = []
        # skal løpe igjennom hvert eneste bilde
        path = os.path.join(self.root, self.raw_dirname, self.fold ,self.gt_dirname)
        print(path)
        sys.exit()
        ...
        for file in files:
            ...
            image_info = {
                ...
                "pixelsize": METADATA_DICT["pixelsize"],
                "mean": np.array(METADATA_DICT["mean"]),
                "std": np.array(METADATA_DICT["std"]),
                }
    ...
    def get_processed_relative_paths(self, tile_info_list):
        ...
        for tile_info in tile_info_list:
            processed_tile_relative_dirpath = os.path.join(f"{tile_info['number']}")
    ...
    def process(self, tile_info_list):
        ...
        for tile_info in tile_info_list:
            print("{}/{}".format(counter, len(tile_info_list)))
            stats = self._process_one(tile_info)
    ...
    def load_raw_data(self, tile_info):
        ...
        # Image:
        raw_data["image_filepath"] = os.path.join(self.root, self.raw_dirname, self.fold, IMAGE_DIRNAME, IMAGE_FILENAME_FORMAT.format(number=tile_info["number"]))
        ...
        # Annotations:
        if self.gt_source == "disk":
            gt_base_filepath = os.path.join(self.root, self.raw_dirname, self.fold, self.gt_dirname, self.gt_type, IMAGE_NAME_FORMAT.format(number=tile_info["number"]) +"_mask_polygonized" )
    ...
    def _process_one(self, tile_info):
        ...
        # --- Init
        tile_name = IMAGE_NAME_FORMAT.format(number=tile_info["number"])
        processed_tile_relative_dirpath = os.path.join(f"{tile_info['number']}")
        processed_tile_dirpath = os.path.join(self.processed_dirpath, processed_tile_relative_dirpath)
        ...
        if self.patch_size is not None:
            ...
            for i, bbox in enumerate(tqdm(patch_boundingboxes, desc=f"Patching {tile_name}", leave=False, position=process_id)):
                ...
                if self.gt_type == "npy" or self.gt_type == "geojson":
                    patch_gt_polygons = polygon_utils.patch_polygons(raw_data["gt_polygons"], minx=bbox[1], miny=bbox[0],maxx=bbox[3], maxy=bbox[2])
            ...
            if not self.mask_only:
                if len(class_freq_list):
                    ...
                    print("Empty tile:", tile_info["number"], "polygons:", len(raw_data["gt_polygons"]))
        else:
    ...
    def __getitem__(self, idx):
        if self.pre_process:
            ...
            if self.mask_only:
                ...
                data["image_mean"] = np.array(METADATA_DICT["mean"])
                data["image_std"] = np.array(METADATA_DICT["std"])
                data["class_freq"] = self.stats["class_freq"]
        else:
            tile_info = self.tile_info_list[idx]
            # Load raw data
            data = self.load_raw_data(tile_info)
            data["name"] = IMAGE_NAME_FORMAT.format(number=tile_info["number"])
...
def main():
    ...
    fold = "test"
    if fold == "train":
        dataset = Autokart(root_dir, fold="train", tile_filter=train_tile_filter, **kwargs)
    elif fold == "val":
        dataset = Autokart(root_dir, fold="val", tile_filter=val_tile_filter, **kwargs)
    elif fold == "test":
        dataset = Autokart(root_dir, fold="test", **kwargs)
    ...
    for batch in tqdm(data_loader):
        ...
        im = np.array(batch["image"][0])
        im = np.moveaxis(im, 0, -1)
        skimage.io.imsave(dir + 'im.png', im)
        ...
        if "gt_polygons_image" in batch:
            ...
            skimage.io.imsave(dir + "gt_seg.png", seg_display)
